// NoteValue will be imported from types.ts, which now correctly infers it
// from THIS file's NOTES constant AFTER it's declared with 'as const'.
import { ScaleDefinition, ChordDefinition } from './types'; // NoteValue will be derived

export const NOTES = ['A', 'A#', 'B', 'C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#'] as const;
// Now NoteValue can be correctly inferred in types.ts as typeof NOTES[number]

// Define NoteValue locally for this file if preferred, or rely on import from types.ts
...
export const SCALES: Record<string, ScaleDefinition> = {
  ...
  minorNatural: { name: 'Natural Minor', intervals: [0, 2, 3, 5, 7, 8, 10] },
  pentatonicMinor: { name: 'Pentatonic Minor', intervals: [0, 3, 5, 7, 10] },
};

export const CHORDS: Record<string, ChordDefinition> = {
  major: { name: 'Major', intervals: [0, 4, 7], quality: 'Stable, Bright' },
  minor: { name: 'Minor', intervals: [0, 3, 7], quality: 'Somber, Reflective' },
  diminished: { name: 'Diminished', intervals: [0, 3, 6], quality: 'Tense, Unstable' },
  augmented: { name: 'Augmented', intervals: [0, 4, 8], quality: 'Suspenseful, Ethereal' },
  ...
  minorMajor7: { name: 'm(maj7)', intervals: [0, 3, 7, 11], quality: 'Mysterious, Cinematic' },
  ...
  sixNine: { name: '6/9', intervals: [0, 4, 7, 9, 14], quality: 'Jazzy, Full' },
...
export const UNIQUE_NOTE_COLORS: Record<NoteValue, string> = {
    'A':  'bg-red-500', 'A#': 'bg-orange-500', 'B':  'bg-yellow-400',
    'C':  'bg-lime-500', 'C#': 'bg-green-500', 'D':  'bg-teal-500',
    'D#': 'bg-cyan-500', 'E':  'bg-blue-500', 'F':  'bg-indigo-500',
    'F#': 'bg-purple-500', 'G':  'bg-fuchsia-500', 'G#': 'bg-pink-500',
};

export const UNIQUE_NOTE_TEXT_COLORS: Record<NoteValue, string> = {
    'A':  'text-white', 'A#': 'text-white', 'B':  'text-black',
    'C':  'text-black', 'C#': 'text-white', 'D':  'text-white',
    'D#': 'text-white', 'E':  'text-white', 'F':  'text-white',
    'F#': 'text-white', 'G':  'text-white', 'G#': 'text-white',
};
